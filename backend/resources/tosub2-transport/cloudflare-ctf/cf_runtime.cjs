...
window.addEventListener("message", (event) => {
  ...
  const bridgeEvent = data && data.source === "cloudflare-challenge" &&
    ["init", "requestExtraParams", "extraParams", "execute", "complete", "token"].includes(data.event);
  ...
  // jsdom marks the synthetic parent/iframe bridge event as untrusted. Only
  // accept the narrow Cloudflare protocol messages that the local bridge
  // creates, then normalize the browser-visible source and origin.
  if (!activeFrameWindow || (!event.isTrusted && !bridgeEvent)) return;
  if (event.source !== activeFrameWindow) defineValue(event, "source", activeFrameWindow);
  if (!event.origin || bridgeEvent) defineValue(event, "origin", "https://challenges.cloudflare.com");
  window.__cfFlow.push({
    messagePatched: true,
