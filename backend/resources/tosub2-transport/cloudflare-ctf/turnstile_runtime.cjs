...
window.addEventListener("message", (event) => {
  ...
  // Messages supplied by the parent runtime are synthetic in jsdom. Accept
  // only the narrow Cloudflare protocol events and normalize their browser
  // visible source/origin before the Turnstile script handles them.
  if (!event.isTrusted && !bridgeEvent) return;
  if (event.source !== outer.window) defineValue(event, "source", outer.window);
  if (!event.origin || bridgeEvent) defineValue(event, "origin", new URL(input.parentUrl).origin);
}, true);
