...
def read_runtime_lines(
    source_name: str,
    process: subprocess.Popen[str],
    events: queue.Queue,
) -> None:
    """Read a Node runtime pipe without relying on select() for Windows pipes."""
    stdout = process.stdout
    if stdout is None:
        events.put((source_name, process, None))
        return
    try:
        for line in stdout:
            events.put((source_name, process, line))
    finally:
        events.put((source_name, process, None))


def read_runtime_stderr(
    source_name: str,
    process: subprocess.Popen[str],
    diagnostics: queue.Queue,
) -> None:
    """Drain runtime stderr so a noisy Node process cannot block on its pipe."""
    ...
    for line in stderr:
        text = line.strip()
        if text:
            diagnostics.put((source_name, text))


def runtime_summary(result: dict[str, Any]) -> str:
    """Return bounded, token-free runtime counters for timeout diagnostics."""
    if not result:
        return "not-finished"
    errors = result.get("errors") or []
    return (
        f"requests={len(result.get('requests') or [])}, "
        f"frames={len(result.get('frames') or [])}, "
        f"frameMessages={len(result.get('frameMessages') or [])}, "
        f"errors={len(errors)}"
    )
...
def solve_challenge(
    ...
    *,
    ...
    diagnostics: queue.Queue = queue.Queue()
    reader_threads: list[threading.Thread] = []
    stderr_threads: list[threading.Thread] = []

    def attach_reader(source_name: str, process: subprocess.Popen[str]) -> None:
        reader = threading.Thread(
            target=read_runtime_lines,
            args=(source_name, process, events),
            name=f"cloudflare-runtime-{source_name}",
            daemon=True,
        )
        reader.start()
        reader_threads.append(reader)

    def attach_stderr_reader(source_name: str, process: subprocess.Popen[str]) -> None:
        reader = threading.Thread(
            target=read_runtime_stderr,
            args=(source_name, process, diagnostics),
            name=f"cloudflare-runtime-stderr-{source_name}",
            daemon=True,
        )
        reader.start()
        stderr_threads.append(reader)
    ...
    try:
        ...
        parent_input = {
            ...
            # Keep the parent alive while the separately booted Turnstile
            # child finishes its message round-trip.
            "waitMs": (timeout_seconds + 5) * 1000,
            "performanceEntries": [
        ...
        attach_reader("parent", parent)
        attach_stderr_reader("parent", parent)
        ...
        active_sources = {"parent"}
        deadline = time.monotonic() + timeout_seconds

        while time.monotonic() < deadline:
            remaining = max(0.05, min(0.5, deadline - time.monotonic()))
            try:
                source_name, process, line = events.get(timeout=remaining)
            except queue.Empty:
                continue
            if line is None:
            ...
            if source_name == "parent" and message_type == "frame-message":
                ...
                if isinstance(data, dict):
                    ...
                    if event_name in {"init", "extraParams", "execute"}:
                        parent_events[event_name] = data

            if source_name == "child" and message_type == "parent-message":
                data = message["message"].get("data")
                event = data.get("event") if isinstance(data, dict) else None
                if event not in {"init", "requestExtraParams"} and parent.poll() is None:
            ...
            if not child_started and frame_url and {"init", "extraParams", "execute"}.issubset(parent_events):
                ...
                active_sources.add("child")
                attach_reader("child", child)
                attach_stderr_reader("child", child)
                child_started = True
        ...
        while True:
            try:
                source_name, line = diagnostics.get_nowait()
            except queue.Empty:
                break
            if len(stderr_lines) < 4:
                stderr_lines.append(f"{source_name}: {line[:240]}")
        detail = "; ".join(stderr_lines)
        summaries = (
            f"parent[{runtime_summary(parent_final)}], "
            f"child[{runtime_summary(child_final)}], "
            f"parentEvents={','.join(sorted(parent_events)) or '<none>'}"
        )
        if detail:
            summaries += f"; stderr={detail}"
        raise RuntimeError(
            "Cloudflare challenge did not issue cf_clearance before timeout; "
            + summaries
        )
    finally:
